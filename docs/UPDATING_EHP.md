...
Create a pull request to Master from a fork. The historical `f2pehp2` preview app is currently not in service.

When the code looks ready to deploy, merge into master, then run the queries listed below against the production Railway deployment backed by PostgreSQL.
