...
RSpec.describe Hiscores do
  describe '.parse_stats_csv' do
    context 'with valid CSV data' do
      ...
      it 'does not flag F2P player with nonzero PvP Arena - Rank' do
        # Test case for the bug fix: PvP Arena Rank can be nonzero on F2P accounts
        csv_data = [
          '12345,750,15000000',    # Overall
          '10000,60,300000',       # Attack
          '10001,60,300000',       # Defence
          ...
          '-1,0',                  # Bounty Hunter (Legacy) - Hunter
          '-1,0',                  # Bounty Hunter (Legacy) - Rogue
          '-1,0',                  # Clue Scrolls (all)
          '-1,0',                  # Clue Scrolls (beginner)
          '-1,0',                  # Clue Scrolls (easy) - P2P
          '-1,0',                  # Clue Scrolls (medium) - P2P
          '-1,0',                  # Clue Scrolls (hard) - P2P
          '-1,0',                  # Clue Scrolls (elite) - P2P
          '-1,0',                  # Clue Scrolls (master) - P2P
          '-1,0',                  # LMS - Rank
          '2500,2500',             # PvP Arena - Rank (nonzero, but should NOT flag as P2P!)
          '-1,0',                  # Soul Wars Zeal
          '-1,0',                  # Rifts closed
          '-1,0',                  # Colosseum Glory
        ...
        # PvP Arena Rank should be stored
        expect(result[:pvp_arena_rank_score]).to eq(2500)
        expect(result[:pvp_arena_rank_rank]).to eq(2500)
        
        # Most importantly: should NOT flag as P2P
        expect(result["potential_p2p"]).to eq(0)
      end

      it 'does not flag F2P player with nonzero Collections Logged' do
        # Test case for the bug fix: Collections Logged can be nonzero on F2P accounts
        csv_data = [
          '12345,750,15000000',    # Overall
          '10000,60,300000',       # Attack
          '10001,60,300000',       # Defence
          ...
          '-1,0',                  # Rifts closed
          '-1,0',                  # Colosseum Glory
          '1,1',                   # Collections Logged (nonzero, but should NOT flag as P2P!)
        ].join("\n")

        result = Hiscores.send(:parse_stats_csv, csv_data)

        # Collections Logged should be stored
        expect(result[:collections_logged_score]).to eq(1)
        expect(result[:collections_logged_rank]).to eq(1)
        
        # Most importantly: should NOT flag as P2P
        expect(result["potential_p2p"]).to eq(0)
      end

      it 'does not flag F2P player with both PvP Arena and Collections Logged nonzero' do
        # Test the exact case from the bug report: F2P Barry
        csv_data = [
          '12345,750,15000000',    # Overall
          '10000,60,300000',       # Attack
          '10001,60,300000',       # Defence
          ...
          '-1,0',                  # Clue Scrolls (master) - P2P
          '-1,0',                  # LMS - Rank
          '2500,2500',             # PvP Arena - Rank = 2500 (from bug report)
          '-1,0',                  # Soul Wars Zeal
          '-1,0',                  # Rifts closed
          '-1,0',                  # Colosseum Glory
          '1,1',                   # Collections Logged = 1 (from bug report)
        ].join("\n")
        ...
        # Most importantly: should NOT flag as P2P despite both being nonzero
        expect(result["potential_p2p"]).to eq(0)
      end
    end
  ...
  describe '.parse_stats' do
    context 'with valid JSON data' do
      ...
      it 'does not flag F2P player with nonzero PvP Arena - Rank' do
        ...
        json_data = {
          'skills' => [
            ...
            { 'name' => 'Construction', 'rank' => -1, 'level' => 1, 'xp' => 0 },
            { 'name' => 'Sailing', 'rank' => -1, 'level' => 1, 'xp' => 0 },
            # PvP Arena with nonzero score
            { 'name' => 'PvP Arena - Rank', 'rank' => 2500, 'score' => 2500 }
          ]
        }
      ...
      it 'does not flag F2P player with nonzero Collections Logged' do
        ...
        json_data = {
          'skills' => [
            ...
            { 'name' => 'Construction', 'rank' => -1, 'level' => 1, 'xp' => 0 },
            { 'name' => 'Sailing', 'rank' => -1, 'level' => 1, 'xp' => 0 },
            # Collections Logged with nonzero score
            { 'name' => 'Collections Logged', 'rank' => 1, 'score' => 1 }
          ]
        }
      ...
      it 'does not flag F2P player with both PvP Arena and Collections Logged nonzero' do
        # Test the exact case from the bug report: F2P Barry (JSON format)
        json_data = {
          'skills' => [
            { 'name' => 'Overall', 'rank' => 12345, 'level' => 750, 'xp' => 15000000 },
            { 'name' => 'Attack', 'rank' => 10000, 'level' => 60, 'xp' => 300000 },
            ...
            { 'name' => 'Construction', 'rank' => -1, 'level' => 1, 'xp' => 0 },
            { 'name' => 'Sailing', 'rank' => -1, 'level' => 1, 'xp' => 0 },
            # Both activities with nonzero scores (from bug report)
            { 'name' => 'PvP Arena - Rank', 'rank' => 2500, 'score' => 2500 },
            { 'name' => 'Collections Logged', 'rank' => 1, 'score' => 1 }
          ]
        }
